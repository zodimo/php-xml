...
class SimpleXMLReader
{
     ...
     */
    public function registerCallback(string $name, $callback, int $nodeType = XMLReader::ELEMENT): SimpleXMLReader
    {
     ...
     */
    public function registerCallbackAtPath(string $name, $callback, string $path, int $nodeType = XMLReader::ELEMENT): SimpleXMLReader
    {
        $clone = clone $this;
    ...
    public function unRegisterCallback(string $name, int $nodeType = XMLReader::ELEMENT): SimpleXMLReader
    {
     ...
     * @throws XmlParserException
     */
    public function unRegisterCallbackAtPath(string $name, string $path, int $nodeType = XMLReader::ELEMENT): SimpleXMLReader
    {
        $clone = clone $this;
        if (!isset($clone->callbacks[$path][$nodeType][$name])) {
        ...
        try {
            while ($continue && $this->reader->read()) {
                ...
                if (XMLReader::END_ELEMENT == $this->reader->nodeType || $this->reader->isEmptyElement) {
                    ...
                    $this->nodePath = preg_replace($pattern, '', $this->nodePath);
                }
            }
        } catch (Throwable $e) {
